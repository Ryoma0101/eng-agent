...
export default function ResultPage() {
  ...
  useEffect(() => {
    if (!submissionId) {
      setError('submissionId が指定されていません。');
      setLoading(false);
      return;
    }

    // TODO: 実際には解答画面から questId / answer を受け取り、このAPI呼び出しに渡す
  ...
  const scoreItems: { key: keyof ScoreDimensions; label: string }[] = [
              ...
              <div className="p-6">
                ...
                <div className="mt-6 flex items-center justify-center">
                  <div className="relative h-40 w-40">
                    ...
                    {/* 4軸の点をスコア比率に応じて配置 */}
                    {scoreItems.map(({ key }, index) => {
                      const value = result.scores[key];
                      const ratio = Math.min(Math.max(value, 0), 25) / 25; // 0-1
